...
if type(_f.func_code) == types.CodeType and __name__ == '__main__': 
    try:
        import _c_2c
    except:
        pass
...
##        print 'ppp', descr
        assert type(descr) in (types.TypeType, str)
        self.descr = descr
        self.subdescr = subdescr
    def __eq__(self, compared):
        if compared == None:
            return False
        if type(compared) in (types.TypeType, str):
            assert False
            return False
    ...
    def __repr__(self):
        ...
        if self.descr == types.ModuleType:
...
def subroutine_can_be_direct(nm, cnt_args):
    if can_be_direct_call(all_co[N2C(nm)].cmds[1]) == True:
        co = N2C(nm)
        if co.co_flags & 0x28 == 0 and len(co.co_cellvars + co.co_freevars) == 0:
...
def disassemble_base(co):
    ...
    while i < n:
        ...
        ## if type(opcmd) == str and opcmd in codes:
            ## opcmd = codes[opcmd]
...
def light_opt_at_cmd_level(cmds):
    for i,cmd in enumerate(cmds):
        if cmd[0] == 'LOAD_CONST' and type(cmd[1]) == types.CodeType:
...
def post_disassemble():
    ...
    for x, co, cmds in seq:
        current_co = co
        if Is3(cmds[0][1], 'CalcConstOldClass') and \
           cmds[1][-1] == ('RETURN_VALUE', ('f->f_locals',)):
            parse_old_class_define(cmds[0][1], cmds[1][:-1])
        elif Is3(cmds[0][1], 'CalcConstNewClass') and \
           cmds[1][-1] == ('RETURN_VALUE', ('f->f_locals',)):
            parse_new_class_define(cmds[0][1], cmds[1][:-1])
...
def is_const_value1(v):
    if v[0] == 'CONST':
        return True
    if v[0] == '!LOAD_NAME' and (type(v[1]) == float or v[1] in ('nan', 'inf')):
        return True
    if v[0] == 'CALC_CONST':
        return True
    if v[0] == '!BUILD_LIST':        
...
def parse_new_class_define(nm, seq):
    return parse_class_def(nm, seq)

def parse_old_class_define(nm, seq):
    return parse_class_def(nm, seq)
...
##            Fatal('')
    ...
    for i, v in enumerate(nms):
        ...
        d = this.__dict__
        s += v    
        assert type(this) == types.ModuleType    
...
##    print 'DOT3', nm
    ...
    if '.' in nm:
        ...
        for i in range(1, len(nms)):
            if nms[i] in this.__dict__ and type(this.__dict__[nms[i]]) == types.ModuleType:
                this = d[nms[i]]
                d = this.__dict__
            else:
                this = d[nms[i]]
...
def _process_compare_op(op, a,b):
    ...
    if a[0] == '!PY_SSIZE_T' and b[0] in ('LOAD_CONST', 'CONST') and type(b[1]) == int:
        if op in op_2_c_op and op not in('is', 'is not'):
    ...
    ## if a[0] == '!PY_SSIZE_T':
        ...
        ## elif op in op_2_inv_op:
            ...
            ## if op in op_2_c_op and op not in('is', 'is not'):
               ## return('!BOOLEAN', ('!c_' + op_2_c_op[op] + '_Int', cmd2mem(b), a))
    if b[0] in ('LOAD_CONST', 'CONST') and type(b[1]) == int:
        if int(b[1]) == b[1] and \
            op in op_2_c_op and op not in('is', 'is not'):
            return('!BOOLEAN', ('!c_' + op_2_c_op[op] + '_Int', cmd2mem(a), b[1]))              
...
def SCmp(cmds,i0,pattern,recursive=False):
    ...
    for i,p in enumerate(pattern):
        ...
        if p == 'ju':
            ...
            if type(tempi) == tuple and len(tempi) >= 1 and\
            ...
            if type(tempi) == list and len(tempi) == 1 and tempi[0][0] == '.L':
                continue
            elif tempi[0] == '.L':    
                continue
            return False  
            ...
            if p[0] == '!':
                ...
                if len(tempi) < 2 or not (p[0] == tempi[0] and p[1] == tempi[1]):
                    return False
            continue
        if type(tempi) == int or tempi[0] != p:
            return False
    ## if collect_stat:    
    ...
    def Stmt(self, *t):
        if len(t) == 1 and t[0] == '':
            ...
            if istempref(t[0]) and t[2] in ('Py_True', 'Py_False'):
                ...
                self.append_cline()
                return self    
            if type(t[2]) == str and t[2].endswith('(glob,'):
                t = list(t)
                t[2:3] = [t[2][:-6], 'glob']
                t = tuple(t)
            t = formire_call(t)
...
def label_method_class(co):
    ...
    if Is3(None, ('Method', co.co_name), nm_for_c) and\
       ...
       co.co_argcount > 0 and co.co_cellvars == () and co.co_freevars == () and\
    ...
    if Is3(func, 'UseLabel', labl):        
        ...
        if not is_direct_current:
            if check_recursive_call:
                o += 'Py_LeaveRecursiveCall();'
            o += 'PyTraceBack_Here(f);'
...
def generate_stand_header(l, co, ltemp, typed):
    if ltemp != 0:
...
def generate_header(nm, o, co, ltemp, typed):
    ...
    generate_stand_header(l, co, ltemp, typed)
    s = 'enum{' 
...
def generate_list(lis, o = None):
    ...
    while i < len(lis):
        it = lis[i]
        head = it[0]
        assert type(head) == int or (head[0] != ')' and head[0:2] != ')(')
        if IsBeg(head):
...
def generate_statement(head,it, o):
    ...
    if head == 'RETURN_VALUE':
        ...
        if len(it[1]) == 1 and it[1][0] == 'f->f_locals':
            ...
            return
        ref = Expr1(it[1], o)
        if not istempref(ref) and not ( is_direct_current and ref[0] == 'FAST'):
             o.INCREF(ref) 

        if not is_direct_current:
            while len(try_jump) > 0:
                if try_jump[-1]:
                    if type(try_jump[-1]) == list:
                        o.Comment((')(FINALLY',))
                        o.Stmt('{')
        ...
        if is_direct_current:
            for i,v in enumerate(current_co.co_varnames):
                nmvar = nmvar_to_loc(v)
                if len(ref) != 2 or ref[0] != 'FAST' or ref[1] != nmvar: # and i >= current_co.co_argcount:
                    o.CLEAR('GETLOCAL(' + nmvar + ')')
        for i,v in enumerate(current_co.co_freevars):
            nmvar = nmvar_to_loc(v)
            if len(ref) != 2 or ref[0] != 'LOAD_CLOSURE' or ref[1] != nmvar: # and i >= current_co.co_argcount:
        ...
        if stat_func == func:
            ...
            o += '}'    
        if not is_direct_current:
            o += 'if (tstate->frame->f_exc_type != NULL) {'
    ...
    if head == 'PYAPI_CALL':
        ...
        for i in range(len(iit)):
            if i > 0:
                if type(iit[i]) == tuple and len(iit[i]) > 0:
...
#            Debug('iiy', iit, gen)
    ...
    if head == 'IMPORT_FROM_AS':
        if it[3][0] == 'CONST' and len(it[3][1]) == len(it[4]):
            ...
            for i in range(len(it[4])):
                ref1 = New()
                o.Raw(ref1, ' = PyObject_GetAttr( ', ref, ', ', ('CONST', it[3][1][i]), ');')
                o.Raw('if (', ref1, ' == NULL) {')
                o += 'if (PyErr_ExceptionMatches(PyExc_AttributeError)) {'
    ...
    if head == 'CONTINUE':
        if try_jump_context[-1]:
            if type(try_jump_context[-1]) == list:
                o.Comment((')(FINALLY',))
                o.Stmt('{')
...
##            if else_cod != None:
            ...
            continue
        if it[i][0] == ')(EXCEPT' and len(it[i]) > 1:
            if ((len (it[i][1]) == 2 and type(it[i][1][1]) == int) or len (it[i][1]) == 1) and \
               len(it[i][2]) == 0:
                    iti = it[i]
                    o.Comment(iti)
                    assert not is_direct_current
                    if len (iti[1]) == 2 and type(iti[1][1]) == int:
...
def generate_logical_expr(it, logic = None):
    ...
    if it0 in ('!c_PyCmp_EQ_Int', '!c_PyCmp_NE_Int', '!c_PyCmp_LT_Int', \
        ...
        elif it[2][0] in ('CONST', 'LOAD_CONST') and type(it[2][1]) == int:
            int_t = it[2][1]    
            int_2 = const_to(int_t)   
        ...
        o.extend(o2)     
        if IsInt(t):
            if it0 == '!c_PyCmp_EQ_Int' and type(int_t) == int:
                o.Raw(logic, ' = PyInt_AS_LONG( ', ref, ' ) == ', str(int_t),  ';')
                o.Cls(ref, int_t)
                return o,logic
            elif it0 == '!c_PyCmp_NE_Int' and type(int_t) == int:
                o.Raw(logic, ' = PyInt_AS_LONG( ', ref, ' ) != ', str(int_t), ';')
                o.Cls(ref, int_t)
                return o,logic
            elif it0 == '!c_PyCmp_LT_Int' and type(int_t) == int:
                o.Raw(logic, ' = PyInt_AS_LONG( ', ref, ' ) < ', str(int_t), ';')
                o.Cls(ref, int_t)
                return o,logic
            elif it0 == '!c_PyCmp_LE_Int' and type(int_t) == int:
                o.Raw(logic, ' = PyInt_AS_LONG( ', ref, ' ) <= ', str(int_t), ';')
                o.Cls(ref, int_t)
                return o,logic
            elif it0 == '!c_PyCmp_GT_Int' and type(int_t) == int:
                o.Raw(logic, ' = PyInt_AS_LONG( ', ref, ' ) > ', str(int_t), ';')
                o.Cls(ref, int_t)
                return o,logic
            elif it0 == '!c_PyCmp_GE_Int' and type(int_t) == int:
                o.Raw(logic, ' = PyInt_AS_LONG( ', ref, ' ) >= ', str(int_t), ';')
                o.Cls(ref, int_t)
                return o,logic
        if IsFloat(t):
            if it0 == '!c_PyCmp_EQ_Int' and type(int_t) == int:
                o.Raw(logic, ' = PyFloat_AS_DOUBLE( ', ref, ' ) == ', str(int_t), ';')
                o.Cls(ref, int_t)
                return o,logic
            elif it0 == '!c_PyCmp_NE_Int' and type(int_t) == int:
                o.Raw(logic, ' = PyFloat_AS_DOUBLE( ', ref, ' ) != ', str(int_t), ';')
    ...
    if it0 in ('!c_PyCmp_EQ_String', '!c_PyCmp_NE_String'):
        ...
        if type(it[2]) == str:
            s_t = it[2] 
            s_2 = const_to(s_t)   
        elif it[2][0] == 'CONST' and type(it[2][1]) == str:
            s_t = it[2][1]    
            s_2 = const_to(s_t)   
        else:
            Fatal('', it)
...
def generate_if(it,o):
    ...
    o1, logic = generate_logical_expr(it[0][1])
    o.extend(o1)
    o.Stmt('if (', logic, ') {')
    o.Cls(logic)
    generate_list(it[1], o)
...
## def is_iter_rng(iter):
    ## v = []
    ## rng = {'range':0, 'xrange':0}
    ## is_need = TCmp(iter, v, ('!PyObject_Call', ('!LOAD_BUILTIN', rng), '?', '?'))
    ## if is_need:
        ## if v[1][0] == 'CONST' and len(v[1][1]) > 0 and type(v[1][1][0]) is long:
            ## return False
    ## return is_need    
...
def have_floatmul(a):
    if type(a) is tuple and len(a) > 0 and a != ('NULL',) and \
           type(a[0]) == str and TypeExpr(a) == Kl_Float:
        return True
    if type(a) is tuple:
        ...
        if i < len(aa) - 2 and \
           ...
           TCmp(aa[i+2], v2, ('STORE', ('?',), \
                        (('!PyObject_Call', ('!LOAD_NAME', v[0]), \
                          ('CONST', ()), ('NULL',)),))):  
##            if subroutine_can_be_direct(v[1], 0):
##                _3(v[1], 'ArgCallCalculatedDef', ('CONST', ()))
...
##            #print '=4'
        ...
        if TCmp(s, v, ('UNPUSH', ('!PyObject_Call', 
            ...
            continue
        s_ = []
        if type(s) == tuple and len(s) > 1 and s[0] == 'UNPUSH' and s[1][0] == '!PyObject_Call' and\
...
def calc_expr_2(op, v1, v2, ret):
    try:
        if op == '!PyNumber_Multiply':
            ...
            if type(v1) == int and v2 != 0:
                return ('CONST', v1 % v2)
            if type(v1) == float and v2 != 0:
                return ('CONST', v1 % v2)
            if type(v1) == str and len(v1 % v2) <= 255:
                return ('CONST', v1 % v2)
            if type(v1) == int and v2 == 0:
                return ('!?Raise', ('BUILTIN', 'ZeroDivisionError'), ('CONST', "division by 0"))
            if type(v1) == float and v2 == 0:
            ...
            ## if type(ret2[1]) == tuple and len(ret2[1]) > 10000:
                ## return ret 
            return ret2
...
def call_calc_const(func, tupl, ret):
    ...
    if subroutine_can_be_direct(func, len(tupl[1])):
        ...
        return ('!CALL_CALC_CONST', func, tupl)
         ...
         len(co.co_cellvars + co.co_freevars) == 0 and co.co_argcount == len(tupl[1]):
...
def repl(ret):
    ...
    if type(ret) is tuple and len(ret) >= 1:
        ...
        if len(ret) == 2 and r0 == 'CONST' and type(ret[1]) == types.TypeType:
        ...
        if r0 == '!PyObject_Call':
            ...
            if direct_call and can_generate_c(current_co):
                if TCmp(ret, v, \
                    ...
                    t = TypeExpr(v[0])
                    if t != None and t.is_old_class_inst() and \
                            Is3(t.subdescr, 'CalcConstOldClass') and\
                            Is3(t.subdescr, ('Method', v[1])):
                        if Is3(t.subdescr, ('ClassMethod', v[1])):
                            Debug('No direct call of class method', ret)
                        elif Is3(t.subdescr, ('StaticMethod', v[1])):
                            tupl = ('!BUILD_TUPLE', v[2])
                            return call_calc_const(Val3(t.subdescr, ('Method', v[1])), tupl, ret)
                        else:                        
                            tupl = ('!BUILD_TUPLE', (v[0],) + v[2])
                            return call_calc_const(Val3(t.subdescr, ('Method', v[1])), tupl, ret)
                    elif t != None and t.is_old_class_typ() and \
                            Is3(t.subdescr, 'CalcConstOldClass') and\
                            Is3(t.subdescr, ('Method', v[1])):
                        if Is3(t.subdescr, ('ClassMethod', v[1])):
                            tupl = ('!BUILD_TUPLE', (v[0],) + v[2])
                    ...
                    elif t != None and t.is_new_class_inst() and \
                            Is3(t.subdescr, 'CalcConstNewClass') and\
                            Is3(t.subdescr, ('Method', v[1])):
                        if Is3(t.subdescr, ('ClassMethod', v[1])):
                            Debug('No direct call of class method', ret)
                        elif Is3(t.subdescr, ('StaticMethod', v[1])):
                            ...
                            tupl = ('!BUILD_TUPLE', (v[0],) + v[2])
                            return call_calc_const(Val3(t.subdescr, ('Method', v[1])), tupl, ret)
                    elif t != None and t.is_new_class_typ() and \
                            Is3(t.subdescr, 'CalcConstNewClass') and\
                            Is3(t.subdescr, ('Method', v[1])):
                        if Is3(t.subdescr, ('ClassMethod', v[1])):
                            tupl = ('!BUILD_TUPLE', (v[0],) + v[2])
...
# TypeError: 'int' object is not iterable !!!!!!!!
        ...
        if r0 == 'PY_TYPE':
            if TCmp(ret, v, ('PY_TYPE', '?', None, ('!PyNumber_Add', '?', '?'), '?')) and v[0] == int:
                n1 = v[1]
                n2 = v[2]
                t = v[3]
            ...
            if TCmp(ret, v, ('PY_TYPE', '?', None, ('!PyNumber_Subtract', '?', '?'), '?')) and v[0] == int:
                n1 = v[1]
                n2 = v[2]
                t = v[3]
                if n1[0] not in ('PY_TYPE', 'CONST'):
            ...
            if TCmp(ret, v, ('PY_TYPE', '?', None, ('!PyNumber_Negative', '?'), '?')) and v[0] == int:
                n1 = v[1]
                t = v[2]
                if n1[0] not in ('PY_TYPE', 'CONST'):
                    n1 = ('PY_TYPE', int, None, n1, t)
        ...
        if r0 == '!c_PyCmp_EQ_String' and \
                TCmp(ret, v, ('!c_PyCmp_EQ_String', ('CONST', '?'), '?')):
...
def is_right_const_value(v2):
    if type(v2) in (int,long,bool,complex,float):
        return True
    ## if type(v2) == tuple and len([x for x in v2 if not is_right_const_value(x)]) == 0:
        ## return True
    return False
...
#        ret = calc_const_to((v[0][0], v[1]))
    ...
    if type(ret) == tuple and len(ret) == 2 and \
...
def TypeExpr(ret):
    ...
    if ret[0] == 'CONST':
        if type(ret[1]) == int and ret[1] >= -30000 and ret[1] <= 30000:
            return Kl_Short
    ...
    if ret[0] == '!PyObject_GetAttr' and ret[2][0] == 'CONST':
        ...
        if t != None:
            if (Is3(t.subdescr, 'CalcConstOldClass') or Is3(t.subdescr, 'CalcConstNewClass')):
                pass
...
#        if (t1,t2) in ((Kl_Boolean, Kl_Int), (Kl_Int, Kl_Boolean)):
        ...
        if t2 == Kl_Tuple and ret[1][0] == '!PySequence_GetSlice':
            return t2        
        return None
    if ret[0] in ('!PyNumber_Multiply', '!PyNumber_Divide', '!PyNumber_Subtract'):
        t1 = TypeExpr(ret[1])
        t2 = None
        if t1 == Kl_Float:
            if t2 == None:
    ...
    if type(ret[0]) == tuple or ret[0] == 'NULL':
        Fatal ('Not tag', ret)
...
def upgrade_op2(ret, nm = None):    
    v = []
##    cnt = sys.getrefcount(0)
    if direct_call and can_generate_c(current_co):
#    if direct_call:
##        if sys.getrefcount(0) > cnt: print '<> -2', sys.getrefcount(0) - cnt
...
##            if sys.getrefcount(0) > cnt: print '<> 3', sys.getrefcount(0) - cnt
...
##            if sys.getrefcount(0) > cnt: print '<> 4', sys.getrefcount(0) - cnt
    elif TCmp(ret, v, ('!PyObject_Call', ('CALC_CONST', ':CalcConstNewClass:'),\
                ('!BUILD_TUPLE', '?'), ('NULL',))):
            ret = ('!CLASS_CALC_CONST_NEW', v[0][0], ('!BUILD_TUPLE', v[1]))
##            if sys.getrefcount(0) > cnt: print '<> 5', sys.getrefcount(0) - cnt
    elif TCmp(ret, v, ('!PyObject_Call', ('CALC_CONST', ':CalcConstNewClass:'),\
                ('CONST', '?'), ('NULL',))):
            ret = ('!CLASS_CALC_CONST_NEW', v[0][0], ('CONST', v[1]))
##            if sys.getrefcount(0) > cnt: print '<> 6', sys.getrefcount(0) - cnt
##    if sys.getrefcount(0) > cnt: print '<> 7', sys.getrefcount(0) - cnt
    v = []
    if TCmp(ret, v, ('IMPORT_FROM_AS', '?', ('CONST', -1), ('CONST', '?'), '?')):
        sreti = []
        imp, consts_, stores = v
##        if sys.getrefcount(0) > cnt: print '<> 8', sys.getrefcount(0) - cnt
        for i, reti in enumerate(stores):
            v = []
            if type(reti) == tuple and len(reti) == 2 and \
               reti[0] in ('STORE_NAME', 'STORE_GLOBAL') and \
               reti[1] in all_calc_const:
                reti = ('STORE_CALC_CONST', reti)
            sreti.append(reti)        
##        if sys.getrefcount(0) > cnt: print '<> 9', sys.getrefcount(0) - cnt
        return ret[:4] + (tuple(sreti),)    
    v0 = []
##    if sys.getrefcount(0) > cnt: print '<> 10', sys.getrefcount(0) - cnt
    if len(ret) == 3 and ret[0] == 'SEQ_ASSIGN':
        sreti = []
##        if sys.getrefcount(0) > cnt: print '<> 11', sys.getrefcount(0) - cnt
        for reti in ret[1]:
            v = []
##            if sys.getrefcount(0) > cnt: print '<> 12', sys.getrefcount(0) - cnt
            if type(reti) == tuple and len(reti) == 2 and \
               reti[0] in ('STORE_NAME', 'STORE_GLOBAL') and \
               reti[1] in all_calc_const:
                calc_const_value[v[0][0]] = ret[2]
                reti = ('STORE_CALC_CONST', reti)
##                if sys.getrefcount(0) > cnt: print '<> 13', sys.getrefcount(0) - cnt
            elif reti[0] == 'SET_VARS':    
                sretii = []
                for retii in reti[1]:
                    if type(retii) == tuple and len(retii) == 2 and \
                       retii[0] in ('STORE_NAME', 'STORE_GLOBAL') and \
                       retii[1] in all_calc_const:
                        retii = ('STORE_CALC_CONST', retii)
                    sretii.append(retii)
                reti = ('SET_VARS', tuple(sretii))        
##                if sys.getrefcount(0) > cnt: print '<> 14', sys.getrefcount(0) - cnt
            sreti.append(reti)        
##        if sys.getrefcount(0) > cnt: print '<> 15', sys.getrefcount(0) - cnt
        return (ret[0], tuple(sreti), ret[2])    
    elif len(ret) == 3 and ret[0] == 'SET_EXPRS_TO_VARS':    
        sretii = []
##        if sys.getrefcount(0) > cnt: print '<> 16', sys.getrefcount(0) - cnt
        for retii in ret[1]:
            if type(retii) == tuple and len(retii) == 2 and \
                retii[0] in ('STORE_NAME', 'STORE_GLOBAL') and \
                retii[1] in all_calc_const:
                retii = ('STORE_CALC_CONST', retii)
            sretii.append(retii)
##            if sys.getrefcount(0) > cnt: print '<> 17', sys.getrefcount(0) - cnt
##        if sys.getrefcount(0) > cnt: print '<> 18', sys.getrefcount(0) - cnt
...
##        if sys.getrefcount(0) > cnt: print '<> 19', sys.getrefcount(0) - cnt
        return ('STORE', (('STORE_CALC_CONST', ret[1][0]),), ret[2])    
    elif type(ret) == tuple and len(ret) == 2 and \
       ret[0] in ('!LOAD_NAME', '!LOAD_GLOBAL', '!PyDict_GetItem(glob,') and \
       ret[1] in all_calc_const:
        ret = calc_const_to(ret[1])    
##        if sys.getrefcount(0) > cnt: print '<> 20', sys.getrefcount(0) - cnt
    v = []    
##    if sys.getrefcount(0) > cnt: print '<> 21', sys.getrefcount(0) - cnt

    ret = repl_collected_module_attr(ret)    
##    if sys.getrefcount(0) > cnt: print '<> 22', sys.getrefcount(0) - cnt
    ret = repl(ret) 
##    if sys.getrefcount(0) > cnt: print '<> 23', sys.getrefcount(0) - cnt
    return ret        
...
def collect_module_type_attr(nm, acc = []):
    ...
    for k in d.keys():
        v = getattr(this, k)
        if type(v) == types.ModuleType and v == this:
            continue
        t = type(v)
...
def collect_set_attr(ret, nm):    
    ...
    if TCmp(ret, v, ('STORE', (('PyObject_SetAttr', '?', ('CONST', '?')),), ('?',))):
        ...
        if len(v[0]) == 1 and (type(v[0][0][0]) == int or v[0][0][0][0:6] == 'STORE_'):
            pass
...
def tree_pass(a, upgrade_op, up, nm):
    if type(a) is tuple:
        ...
        while True:   
            ...
            notchanged = len(r) == len(a) and all([id(r[i]) == id(a[i]) for i,x in enumerate(r)])
            if notchanged:
                r = a
            ## else:
                ## print 'old', a, 'new', r    
...
def ortogonal(a, upgrade,up=None):
    if type(a) is tuple:
        ...
        if all([r[i] == a[i] for i,x in enumerate(r)]):
            r = a
        while True:   
            r2 = upgrade(r)
            if r2 == r:
...
def replace_subexpr(a,b,c):
    ...
    if type(a) is tuple:
        s = ()
        for i in a:
            s += (replace_subexpr(i,b,c),)
        return s
    if type(a) is list:
        s = []
        for i in a:
            s.append(replace_subexpr(i,b,c))
        return s
    return a    
...
def generate_list_compr_enumerate(acc, val, store_ind, store_val, enumerated, cond, o): 
    ...
    o.Stmt(n_iter, '++')
    if cond != None:
        o1, cond_var = generate_logical_expr(cond[0])
        o.extend(o1)
        o.Stmt('if (', cond_var, '){')
...
def generate_list_compr(val, descr_compr, o, forcenewg):
    ...
    if len(descr_compr) == 3 and len(store) == 2 and len(expr) == 1 and \
        ...
        generate_list_compr_enumerate(acc0, val, store[0], store[1], \
                                      expr[0][2][1][0], cond, o)
        return acc0                             
    ## if 'iteritems' in repr(descr_compr):
        ## pprint.pprint(descr_compr)
    ## if 'iterkeys' in repr(descr_compr):
        ## pprint.pprint(descr_compr)
    ## if 'itervalues' in repr(descr_compr):
        ## pprint.pprint(descr_compr)
...
#                o.INCREF(ref)
            ...
            if type(ind) == int:
                if ind < 0:
                    ind2 = New('long')
                    o.Stmt(ind2, '=', ind, '+', 'PyList_GET_SIZE(' + CVar(ref1) + ')')
                    ind = ind2
                else:
                    pass    
            else:    
...
def handle_unpack_except(o, src_len, trg_len):
    ...
    o += 'PyErr_Format(PyExc_ValueError, "too many values to unpack");'
    o.Raw('goto ', labl, ';')
    UseLabl()
    o += '}'    
    o.Raw('if (', src_len, ' < ', trg_len, ') {')
    ...
    ## o.append('if (' + CVar(ref1) + ' == NULL) {')
    ## o.append('if (PyErr_ExceptionMatches(PyExc_AttributeError)) {')
    ## o.append('PyErr_Format(PyExc_ValueError, ' + ('"cannot import name %s"' % it[3][1][i]) + ');')
    ## o += '}'
    ## o.append('goto ' + CVar(labl) + ';')
    ## UseLabl()
    ## o += '}'    
...
def GenExpr(it,o, forcenewg=None,typed=None, skip_float = None):
    ...
    if TCmp(it, _v, ('!PyObject_Call', ('!PyObject_GetAttr', '?', ('CONST', '?')), '?', '?')):
        ...
        if t != None and _v[3] == ('NULL',):
            if t not in _Kl_Simples:
                if ((Is3(t.subdescr, 'CalcConstOldClass') or \
                   Is3(t.subdescr, 'CalcConstNewClass')) and \
                     not Is3(t.subdescr, ('Attribute', _v[1]))) \
                       or t.subdescr == None:
...
#            Debug('Undefined class method', it)            
    ...
    if not isinstance(head, types.StringTypes) and type(head) != int:
        pprint.pprint(head)
        pprint.pprint(it)
    assert isinstance(head, types.StringTypes) or type(head) == int
#    o.append('/*---*/')
    tempor = False    
...
#        return GenExpr(('!PyDict_GetItem', 'glob', ('CONST', it[1])),o, forcenewg)
...
#        Debug('Generate builtin: %s' % it[1])
    ...
    if head in ('AND_JUMP', 'OR_JUMP'):
        o2,logic = generate_logical_expr(it)
        o.extend(o2)
        ref = New(None, forcenewg)
...
#### Trouble at count < 0 
        if it[1][0] == 'CONST' and type(it[1][1]) == str and\
           len(it[1][1]) == 1 and TypeExpr(it[2]) == Kl_Int:
            ...
            ref2 = New(None,forcenewg)
            o.Raw('if (', n2, ' <= 0) {')
##            o +='PyErr_SetString(PyExc_OverflowError, "count of repeat too small");'
##            o.Raw('goto ', labl, ';')
##            UseLabl()
...
##            ref2 = New(None,forcenewg)
    ...
    if head == 'BUILD_MAP':
      ...
      for k, v in it[1]:
          ...
          o.Cls(k, v)
        ## for k,v in ((Expr1(x[0], o), Expr1(x[1], o)) for x in it[1]):
##            o.Stmt('PyDict_SetItem', newg, k, v)
##         o.Cls(k, v)
      return newg
    if head == 'MK_CLOSURE':
    ...
    if head == 'MK_FUNK':
        ...
        if len(it) == 3 and it[2][0] == 'CONST' and type(it[2][1]) is tuple:
            ...
            ## elif all_co[co].simple_cmds:
                ## o.Stmt(ref1, '=', 'Py2CFunction_New_Simple', const_to( co), '&Py2CFunction_'+it[1]+'_Type')
            else:
                o.Stmt(ref1, '=', 'Py2CFunction_New', const_to( co))
    ...
    if head == 'BINARY_SUBSCR_Int':
        ...
        if expand_BINARY_SUBSCR or islist:
            ...
            ref2 = Expr1(it[2], o)
            o.Raw(ref1, ' = PyDict_GetItem(', ref, ', ', ref2, ');')
            o.Raw('if(', ref1, '== 0) {')
            tup = New()
            o.Raw(tup, ' = PyTuple_Pack(1, ', ref2, ');')
    ...
    if head == 'from_ceval_BINARY_SUBSCR' and it[2][0] == '!@PyInt_FromSsize_t':
        ...
        if t == Kl_Dict:
            ref2 = Expr1(it[2][2], o)
            ## o.Stmt(ref1, '=', 'PyDict_GetItem', ref, ref2)
            ## o.Cls(ref, ref2)
            o.Raw(ref1, ' = PyDict_GetItem(', ref, ', ', ref2, ');')
            o.Raw('if(', ref1, '== 0) {')
            tup = New()
    ...
    if head == '_PyList_Pop':
        ...
        if len(it) == 3:
            if it[2][0] == 'CONST':
                ind1 = New('long')
                if it[2][1] < 0:
                    o.Stmt(ind1, '=', 'PyList_GET_SIZE', ref0)
                    o.Stmt(ind1, '=', ind1, '-', abs(it[2][1]))
                else:    
                    o.Stmt(ind1, '=', it[2][1]    )
            else:
                o2,ind1 = generate_ssize_t_expr(it[2])
...
##        o.append
    ...
    if head in ('c_PyCmp_EQ_Int', 'c_PyCmp_NE_Int', 'c_PyCmp_LT_Int', 'c_PyCmp_LE_Int', 'c_PyCmp_GT_Int', 'c_PyCmp_GE_Int'):
        ...
        if type(it[2]) == int:
            int_t = it[2] 
            int_2 = const_to(int_t)   
        elif it[2][0] == 'CONST' and type(it[2][1]) == int:
            int_t = it[2][1]    
            int_2 = const_to(int_t)   
        elif it[2][0] in ('!PY_SSIZE_T',):
    ...
    if head == 'CHR_BUILTIN':
        ...
        o += 'PyErr_SetString(PyExc_ValueError, \"chr() arg not in range(256)\");'
        o.Raw('goto ', labl, ';')
        UseLabl()
        o += '}'
        o += '{'
...
#        print it
    ...
    if head == 'BOOLEAN':
        o2,logic = generate_logical_expr(it[1])
        o.extend(o2)
        ref = New(None, forcenewg)
        o.Stmt(ref, '=','PyBool_FromLong', logic)
        o.Cls(logic)
    ...
    if head == '1NOT' and it[1][0] == '!BOOLEAN':
        o1, int_val = generate_logical_expr(it[1])
        o.extend(o1)
        ref = New(None, forcenewg)
        o.Stmt(int_val, '=', '!', int_val) ## ????????? 
        o.Stmt(ref, '=','PyBool_FromLong', int_val)
        o.Cls(int_val)
        return ref
...
#        o.Cls(ref1)
    ...
    if head == 'CALL_CALC_CONST_INDIRECT':
        ...
        ## ref1 = Expr1(it[2], o)
        ## ref = New(None, forcenewg)
        ## o.Raw(ref, ' = InDirect_Call_' + it[1] + '(', ref1, ');') 
        ## o.Cls(ref1)
        ## o.Raw('if (', ref, ' == NULL) goto ', labl, ';')
        ## UseLabl()
        ## return ref
    ...
    if head == '?Raise':
        gen = [Expr1(x, o) if type(x) == tuple and len(x) > 0 else x \
...
#            o.Stmt(add, '=', 'PyObject_IsTrue', r2)
    ...
    if head in ('!PyNumber_Subtract', '!PyNumber_Add'):
...
## def to_long(o, ref1, it):
    ...
    ## if len(last) > 0:
        ...
        ## if last[-1] == che:
            ...
            ## del last[-1]
    ## if len(last) > 0:
        ## beg = CVar(ref1) + ' = PyInt_FromSsize_t ( '
        ## if last[-1].startswith(beg) and last[-1].endswith(' );'):
            ## if del_check:
                ## del o[-1]
            ## c_expr = o[-1][len(beg):-3]
            ## del o[-1]
            ## o.ClsFict(ref1)
            ## return c_expr 
        ## beg = CVar(ref1) + ' = PyInt_FromLong('
...
def genHalfFloat(it1, o):
    if it1[0] == 'CONST' and type(it1[1]) == float:
        return repr(it1[1])
...
##    l_o = len(o)
    ## if it[0] == '!BOOLEAN' or (it[0] == '!1NOT' and it[1][0] == '!BOOLEAN'):
        ## if it[0] == '!BOOLEAN':
            ## iit = it[1]
        ## elif (it[0] == '!1NOT' and it[1][0] == '!BOOLEAN'):
            ## iit = it[1][1]
        ## else:
            ## Fatal ('', it)   
        ## o1, int_val = generate_logical_expr(iit)
        ## o.extend(o1)

        ## assert is_and == True or is_and == False
        ## or_and = New('int')
        ## if is_and:    
            ## o.Stmt(or_and, ' = ', int_val, ';')
        ## else:    
            ## o.Stmt(or_and, ' = !', int-val, ';')
...
##    return tag in repr(expr)
    if type(expr) == tuple and len(expr) > 0:
        if expr[0] == tag:
            return True
        if expr[0] == 'CONST':
            return False
...
def generate_consts(cfile):
    ...
    for i, (c,type_c) in enumerate(consts):
        if type(c) == tuple:
            codes = [(j,c1) for j,c1 in enumerate(c) if type(c1) == types.CodeType]
            for j, c1 in codes:
                change_ob = True
    if change_ob:            
    ...
    for i, (c,type_c) in enumerate(consts):
        if type(c) == tuple:
            ...
            for j, c1 in codes:
                ...
                s_inc = 'Py_INCREF(' + s1 + ');'
                s2 = const_to(c)
#                s_set = 'if(PyTuple_SetItem(' + s2 + ', ' + str(j) + ', ' + s1 + ')!= 0) {printf(\"line code %d\\n\", __LINE__);}'
...
#        return load_builtin(nm) 
    ...
    if type(c_[0]) == float:
        for i, (cc, cc_typ) in enumerate(consts):
            if type(cc) == float and math.isinf(cc) and  math.isinf(c_[0]) and str(cc) == str(c_[0]):
                return 'consts[' + str(i) + ']'
            if type(cc)  == float and math.isnan(cc) and  math.isnan(c_[0]) and str(cc) == str(c_[0]):
                return 'consts[' + str(i) + ']'
...
if __name__ == "__main__":
    ...
    ## if type(main.func_code) == types.CodeType: 
        ## try:
            ## import _c_2ctest
        ## except:
            ## pass
